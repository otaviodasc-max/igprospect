...
/* =====================================================================
   CRM
===================================================================== */
// Sigla curta pra identificar um funil no rail lateral do CRM (2-3 letras),
// ex.: "Social Selling"→"SS", "Instagram"→"INS", "Empresários"→"EMP".
function crmSigla(name){
...
/* ---- O que vale dinheiro pra quem prospecta ---------------------------
   Não é só o lead: é cada TRABALHO feito. Chamar um lead vale uma unidade
   no dia do chamado; fazer follow-up num lead chamado meses atrás vale
   outra unidade, no dia do follow-up — porque o follow-up também é tempo
   gasto, e num dia só de follow-up a pessoa não pode ficar sem receber.
   Um mesmo lead pode render várias unidades ao longo do tempo (o chamado +
   cada etapa de follow-up por onde ele passou), mas cada etapa só uma vez:
   stage_dates guarda a PRIMEIRA entrada em cada etapa e nunca reescreve,
   então reabrir a mesma etapa não paga de novo.
   A etapa de follow-up é achada pelo LABEL (qualquer etapa cujo nome tenha
   "follow"), nunca por key fixa — cada equipe nomeia as etapas do seu jeito
   e as keys não são confiáveis entre equipes. */
function isFollowUpStage(st){ return normLabel(st&&st.label).includes('follow'); }
function orgHasFollowUpStage(){ return S.pipelines.some(p=>stagesOf(p).some(isFollowUpStage)); }
// Devolve as unidades pagáveis de UM lead: {date, kind, by}. `kind` separa o
// chamado/seguido (o que conta pra meta diária do bônus) do follow-up.
function leadPayUnits(l, chamados, withFollowUp){
  const out=[];
  if(chamados){ if(leadWasCalled(l)){ const d=leadCalledDate(l); if(d) out.push({ date:d, kind:'base', by:l.createdBy }); } }
  else if(l.addedAt) out.push({ date:l.addedAt, kind:'base', by:l.createdBy });
  if(withFollowUp){
    const pl=leadPipeline(l);
    stagesOf(pl).forEach(st=>{
      if(!isFollowUpStage(st) || !leadReachedStage(l,st.key,pl)) return;
      const d=leadStageDate(l,st.key,pl);
      if(d) out.push({ date:d, kind:'followup', by:l.createdBy });
    });
  }
  return out;
}
...
function prospectPayOf(units, g){
  const base = g.payTargetPerDay>0 ? (g.payDayRate||0)/g.payTargetPerDay : 0;
  ...
  return { pay, perLead:base, bonusDays };
}
// Uma única porta de entrada pro cálculo da prospecção, usada pelo card da
// semana (weeklyPay) e pelo relatório por período (weekReport) — assim os
// dois números nunca divergem.
function prospectReport(inW, memberId){
  const g=getGoals();
  const chamados=g.payBasis==='chamados';
  const withFollowUp=g.payFollowUp;
  const units=[];
  S.leads.forEach(l=>{
    if((l.tipo||'comum')==='empresario') return;
    if(memberId && l.createdBy!==memberId) return;
    leadPayUnits(l, chamados, withFollowUp).forEach(u=>{ if(inW(u.date)) units.push(u); });
  });
  const { pay, perLead, bonusDays }=prospectPayOf(units, g);
  const followUps=units.filter(u=>u.kind==='followup').length;
  return { prospectLeads:units.length-followUps, followUps, prospectUnits:units.length, prospectPay:pay, perLead, bonusDays, payBasis:g.payBasis };
}
...
function weeklyPay(){
  ...
  const rep=prospectReport(inW, rid);
  const { prospectLeads, followUps, prospectUnits, prospectPay, perLead, bonusDays }=rep;
  ...
  return { ws, we, dayRate, target, perLead, prospectLeads, followUps, prospectUnits, prospectPay, bonusDays, unpaid, unpaidTotal, total:prospectPay+unpaidTotal, recipientId:rid, payBasis:g.payBasis };
}
...
function weekReport(ws, we, memberId){
  const inW = iso => { if(!iso) return false; const d=new Date(iso); return d>=ws && d<we; };
  const g=getGoals();
  const { prospectLeads, followUps, prospectUnits, prospectPay, bonusDays }=prospectReport(inW, memberId);
  ...
  return { prospectLeads, followUps, prospectUnits, prospectPay, bonusDays, pending, paid, pendingTotal, total:prospectPay+pendingTotal, dealIds:pending.map(d=>d.id), payBasis:g.payBasis };
}
...
function payBreakdownHtml(rep, idPrefix){
  ...
  return `
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:12px;margin-bottom:14px">
        <div style="background:var(--surf2);border-radius:9px;padding:12px 14px"><div style="font-size:.72rem;color:var(--t3);margin-bottom:3px">Prospecção · ${rep.prospectLeads} leads ${rep.payBasis==='chamados'?'chamados':'seguidos'}${rep.followUps?` + ${rep.followUps} follow-up${rep.followUps>1?'s':''}`:''}${rep.bonusDays?` · <b style="color:#10B981">${rep.bonusDays} dia${rep.bonusDays>1?'s':''} com bônus</b>`:''}</div><div id="${idPrefix}-prospect" data-cnt="${rep.prospectPay}" style="font-family:'Plus Jakarta Sans';font-weight:800;font-size:1.3rem;color:#6366F1">${fmtCurrency(0)}</div></div>
...
// criado), filtrados pela data em que entraram no funil (added_at). Contagem
// exclusiva (quem está ali agora) em toda etapa, exceto a 1ª — ver
// comentário em renderRelDash.
...
function renderRelDash(targetId){
  ...
  // funil pode ter uma quantidade diferente de etapas). Mesma contagem das
  // barras do funil (ver comentário acima — só a 1ª etapa não cai).
  ...
  const statDefs=[ { key:'', label:'Total no período', n:total, color:'#6366F1' }, ...stages.map(s=>({ key:s.key, label:s.label, n:displayCount(s.key), color:s.color })) ];
...
===================================================================== */
// "Excluir equipe" no seletor NÃO sai da equipe nem apaga nada — só some da
// lista deste computador (guardado em localStorage, por navegador/aparelho).
// Continua sendo membro; a equipe volta a aparecer sozinha se um dia virar
// a equipe ativa de novo (ver boot()).
function hiddenOrgIds(){ try{ return new Set(JSON.parse(localStorage.getItem('igp_hidden_orgs')||'[]')); }catch(e){ return new Set(); } }
function isOrgHiddenHere(id){ return hiddenOrgIds().has(id); }
...
async function renderOrgSwitcher(){
  openModal(`<div class="modal-ov"><div class="modal-box" style="max-width:720px"><div class="modal-hd"><div><div class="modal-title">Minhas equipes</div><div class="modal-sub">Selecione a equipe que quer acessar</div></div><div class="x"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></div></div>
    <div class="modal-bd">
      <div class="search-wrap" style="margin-bottom:14px"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg><input class="search-inp" id="org-sw-q" placeholder="Buscar equipe…"></div>
      <div id="org-sw-list" class="org-grid"><div style="font-size:.8rem;color:var(--t3)">Carregando…</div></div>
    </div>
...
function notePresetsModal(){
  // Cada etapa vira um <details> fechado por padrão, em cards retangulares
  // lado a lado (grid horizontal) em vez de uma lista de barras empilhadas —
  // cabe mais etapa por tela e fica mais fácil de achar a que se quer editar.
  // Ao abrir, o card ocupa a linha inteira (via CSS [open]) pra dar espaço
  // de sobra pra digitar na textarea.
  const stageRow=(s,inputAttr)=>{
    const cur=(s.notes||[]).filter(Boolean);
    const n=cur.length;
    return `<details class="np-acc">
      <summary>${esc(s.label)} <span class="np-count">· ${n} observaç${n===1?'ão':'ões'}</span></summary>
      <div class="np-bd"><textarea class="stg-input np-inp" ${inputAttr(s)} placeholder="Uma observação por linha…">${esc(cur.join('\n'))}</textarea></div>
    </details>`;
  };
  const section=(title,sub,stages,inputAttr)=>`<div style="margin-bottom:14px"><div class="stg-ri-t">${esc(title)}</div>${sub?`<div class="stg-ri-s" style="margin-bottom:8px">${esc(sub)}</div>`:''}<div class="np-grid">${stages.map(s=>stageRow(s,inputAttr)).join('')||'<div class="empty-sub">Nenhuma etapa ainda.</div>'}</div></div>`;
  ...
  openModal(`<style>
    .np-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(190px,1fr));gap:8px}
    .np-acc{border:1px solid var(--border2);border-radius:10px;overflow:hidden;align-self:start}
    .np-acc[open]{grid-column:1/-1}
    .np-acc summary{padding:12px 14px;cursor:pointer;font-size:.79rem;font-weight:600;color:var(--t1);background:var(--surf2);list-style:none}
    .np-acc summary::-webkit-details-marker{display:none}
    .np-acc[open] summary{background:var(--surf3)}
    .np-count{font-weight:400;color:var(--t3);font-size:.7rem}
    .np-bd{padding:10px 12px}
    .np-inp{min-height:70px;width:100%;box-sizing:border-box}
  </style>
  <div class="modal-ov"><div class="modal-box"><div class="modal-hd"><div><div class="modal-title">Observações pré-prontas</div><div class="modal-sub">Ao marcar um lead/negociação numa etapa, esses textos aparecem numa lista suspensa pra escolher no campo Observações</div></div><div class="x"><svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></div></div>
...
/* =====================================================================
   BOOT
===================================================================== */
// freshLogin=true SÓ quando vem direto do formulário de login/cadastro (ver
// authSubmit) — é o sinal de "acabou de entrar", diferente de um F5 que
// só retoma a sessão já salva. É o que decide se pergunta qual equipe abrir.
async function boot(freshLogin){
